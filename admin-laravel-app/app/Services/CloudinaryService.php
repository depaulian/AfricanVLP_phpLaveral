...
class CloudinaryService
{
     ...
     */
    public function uploadSupportAttachment(UploadedFile $file, string $ticketId): array
    {
        try {
            $publicId = "africavlp/support/{$ticketId}/" . time() . '_' . pathinfo($file->getClientOriginalName(), PATHINFO_FILENAME);
            
            $options = [
                'public_id' => $publicId,
                'folder' => 'africavlp/support',
                'resource_type' => 'auto',
            ];

            return $this->upload($file, $options);
        } catch (Exception $e) {
            Log::error('Cloudinary support attachment upload failed: ' . $e->getMessage());
            throw $e;
        }
    }
     ...
     */
    public function uploadFeedbackAttachment(UploadedFile $file, string $feedbackId): array
    {
        try {
            $publicId = "africavlp/feedback/{$feedbackId}/" . time() . '_' . pathinfo($file->getClientOriginalName(), PATHINFO_FILENAME);
            
            $options = [
                'public_id' => $publicId,
                'folder' => 'africavlp/feedback',
                'resource_type' => 'auto',
            ];

            return $this->upload($file, $options);
        } catch (Exception $e) {
            Log::error('Cloudinary feedback attachment upload failed: ' . $e->getMessage());
            throw $e;
        }
    }
