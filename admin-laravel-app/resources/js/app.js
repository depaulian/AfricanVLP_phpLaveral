...
Alpine.data('accessibility', accessibility);
Alpine.data('helpSystem', helpSystem);
...
window.showNotification = function(message, type = 'info', duration = 5000) {
    ...
    notification.className = `fixed top-4 right-4 z-50 max-w-sm w-full bg-white rounded-lg shadow-lg border-l-4 ${getNotificationColor(type)} transform translate-x-full transition-transform duration-300 ease-in-out`;
...
window.bulkAction = function(action, selectedItems) {
    if (selectedItems.length === 0) {
        showNotification('Please select at least one item', 'warning');
        return false;
    }
    
    const message = `Are you sure you want to ${action} ${selectedItems.length} item(s)?`;
    return confirm(message);
};

// Auto-save functionality
window.autoSave = function(form, endpoint, interval = 30000) {
    let timeout;
...
function saveForm(form, endpoint) {
