...
class OrganizationTest extends TestCase
{
    ...
    public function test_organization_has_pending_invitations_relationship()
    {
        $organization = Organization::factory()->create();
        $invitation = TmpOrganizationUser::factory()->create([
            'organization_id' => $organization->id,
            'status' => 'pending'
        ]);

        $this->assertTrue($organization->pendingInvitations->contains($invitation));
        $this->assertInstanceOf(\Illuminate\Database\Eloquent\Collection::class, $organization->pendingInvitations);
    }
    ...
    public function test_organization_by_location_scope()
    {
        Organization::factory()->create(['location' => 'New York']);
        Organization::factory()->create(['location' => 'California']);
        Organization::factory()->create(['location' => 'New York']);

        $nyOrgs = Organization::byLocation('New York')->get();
        
        $this->assertCount(2, $nyOrgs);
        $this->assertTrue($nyOrgs->every(fn($org) => $org->location === 'New York'));
    }
    ...
    public function test_organization_can_invite_user()
    {
        $organization = Organization::factory()->create();
        $email = 'newuser@example.com';

        $invitation = $organization->inviteUser($email, 'volunteer');

        $this->assertInstanceOf(TmpOrganizationUser::class, $invitation);
        $this->assertEquals($email, $invitation->email);
        $this->assertEquals('volunteer', $invitation->role);
    ...
    public function test_organization_volunteer_count_accessor()
    {
        $organization = Organization::factory()->create();
        $volunteers = User::factory()->count(2)->create();
        $members = User::factory()->count(1)->create();
        
        foreach ($volunteers as $volunteer) {
            $organization->users()->attach($volunteer->id, ['role' => 'volunteer']);
        }
        ...
        $this->assertEquals(2, $organization->volunteer_count);
    }
    ...
    public function test_organization_volunteer_users_relationship()
    {
        $organization = Organization::factory()->create();
        $volunteerUser = User::factory()->create();
        $memberUser = User::factory()->create();
        
        $organization->users()->attach($volunteerUser->id, ['role' => 'volunteer']);
        $organization->users()->attach($memberUser->id, ['role' => 'member']);

        $volunteers = $organization->volunteerUsers;
        
        $this->assertCount(1, $volunteers);
        $this->assertTrue($volunteers->contains($volunteerUser));
        $this->assertFalse($volunteers->contains($memberUser));
    }
    ...
    public function test_organization_settings_json_casting()
    {
        ...
        $this->assertTrue($organization->settings['volunteer_opportunities']);
    ...
    public function test_organization_can_check_if_user_is_volunteer()
    {
        $organization = Organization::factory()->create();
        $volunteerUser = User::factory()->create();
        $memberUser = User::factory()->create();
        ...
        $this->assertTrue($organization->isVolunteer($volunteerUser->id));
        $this->assertFalse($organization->isVolunteer($memberUser->id));
    }
}