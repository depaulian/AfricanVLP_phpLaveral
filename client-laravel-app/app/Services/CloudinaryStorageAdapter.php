...
class CloudinaryStorageAdapter
{
    ...
    public function store(UploadedFile $file, string $context, array $metadata = []): array
    {
        try {
            switch ($context) {
                ...
                case 'document':
                    return $this->storeDocument($file, $metadata['user_id'], $metadata['type'] ?? 'document');
                
                case 'portfolio_item':
                    return $this->storePortfolioItem($file, $metadata['user_id']);
    ...
    /**
     * Store document/verification file
     */
    protected function storeDocument(UploadedFile $file, string $userId, string $type): array
    {
        $result = $this->cloudinaryService->uploadDocument($file, $userId, $type);
     ...
     */
    protected function storePortfolioItem(UploadedFile $file, string $userId): array
    {
        $result = $this->cloudinaryService->uploadPortfolioItem($file, $userId);
